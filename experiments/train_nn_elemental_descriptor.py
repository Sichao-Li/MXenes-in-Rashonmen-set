...
class Net_ele_descriptor(torch.nn.Module):
    def __init__(self):
        super(Net_ele_descriptor, self).__init__()
        self.fc1 = Linear(20, 64)
        self.fc2 = Linear(64, 32)
        self.fc3 = Linear(32, 32)
        self.fc4 = Linear(32, 5)
...
if __name__ == "__main__":
    train_dataloader, test_dataloader, val_dataloader = preprocess(elemental_descriptor=True)
    ...
    for epoch in range(nb_epochs):
        ...
        if min_valid_loss > valid_loss:
            ...
            torch.save(model.state_dict(), '../results/model/best_model_ANN_ele_descriptor.pth')
        print("Epoch: {} Loss: {} Val_Loss: {}".format(epoch, epoch_loss, valid_loss))
    with open('../results/model/train_loss_ele_descriptor.npy', 'wb') as f:
        np.save(f, train_loss)
    with open('../results/model/test_loss_ele_descriptor.npy', 'wb') as f:
        np.save(f, val_loss)